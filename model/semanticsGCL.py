...
from model.Interaction_module_test import *
...
class unit_gcn(nn.Module):
    ...
    def forward(self, x):
        ...
        return y, graph_list#, torch.stack(graph_list, 1)
...
class Model(nn.Module):
    def __init__(self, num_class=120, num_point=25, num_person=2, graph=None, graph_args=dict(), in_channels=3,
                 drop_out=0, adaptive=True, head=['ViT-B/32']):
    ...
    def forward(self, x, x_object=None, flag='test'):
        if len(x.shape) == 3:
            N, T, VC = x.shape
        ...
        if flag == 'train':
            ...
            for graph in graph_list:
                graph2 = graph.view(N, M, c_new, V, V).mean(1).view(N,c_new,-1)
                graph_base.append(graph2)
                graph_semantic = self.attention(graph2,text_corelation)
                graph_semantic_list.append(graph_semantic)
            graph_base = torch.stack(graph_base, 1).mean(2).view(N,-1)
            graph2 = torch.stack(graph_semantic_list, 1)
            graph2_semantic = graph2.mean(2).view(N,-1)
            return self.fc(x), graph_base , graph2_semantic 
        else: 
            return self.fc(x)
